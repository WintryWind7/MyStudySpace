...
avg_func_create = content.find('CREATE FUNCTION avg_func', five_pos)
delimiter_start = content.rfind('DELIMITER @@', five_pos, avg_func_create)
drop_func = content.find('DROP FUNCTION avg_func', avg_func_create)
drop_para_end = content.find('</w:p>', drop_func) + 6

print(f'DELIMITER开始: {delimiter_start}')
print(f'DROP结束: {drop_para_end}')
