...
five_end = 266203      # 五标题 </w:p> 之后
six_start = 266868     # 六标题 <w:p 开始
six_end = 267322       # 六标题 </w:p> 之后

print(f'五结束: {five_end}, 六开始: {six_start}, 六结束: {six_end}')
...
for line in lines:
    ...
    five_paras.append(make_para(line) if is_sql else make_para_cn(line))

five_xml = ''.join(five_paras)

# 插入五部分：替换 five_end 到 six_start 之间的空段落
new_doc = content[:five_end] + five_xml + content[six_start:]

print(f'添加 {len(five_paras)} 个五部分段落')
...
# 找实验二的六部分
data_query = content2.find('数据查询')
six_text = content2.find('六、实验体会和收获', data_query)
six_p_end = content2.find('</w:p>', six_text) + 6

# 找实验三标题
exp3_text = content2.find('视图和索引', six_p_end)
exp3_p_start = content2.rfind('<w:p ', 0, exp3_text)

print(f'六结束: {six_p_end}, 实验三开始: {exp3_p_start}')
...
print(f'添加 {len(exp_lines)} 个心得段落')
...
print(f'输出: {output}, 大小: {os.path.getsize(output)} bytes')