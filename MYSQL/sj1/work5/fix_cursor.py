...
# 读取document.xml
with open('fix_cursor/word/document.xml', 'r', encoding='utf-8') as f:
    content = f.read()

# 定位avg_func代码块的正确边界
five_pos = 493977  # 实验五五部分位置

# 找avg_func题目段落："5.创建存储函数avg_func，通过游标统计指定课程的平均分。"
...
# 写回
with open('fix_cursor/word/document.xml', 'w', encoding='utf-8') as f:
    f.write(new_content)
...
with zipfile.ZipFile(output, 'w', zipfile.ZIP_DEFLATED) as zf:
    for root, dirs, files in os.walk('fix_cursor'):
        for file in files:
            file_path = os.path.join(root, file)
            arc_path = os.path.relpath(file_path, 'fix_cursor')
