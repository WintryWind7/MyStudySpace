...
# Step 1: Unpack template fresh
src = '../template/实验一done.docx'
dst = '../unpacked_exp2_v2'
...
# Find experiment 2 sections
exp2_pos = content.find('实验项目名称：     数据查询')
print(f'实验二标题位置: {exp2_pos}')

# Find 五 section for exp2 (after exp2_pos)
five_exp2 = content.find('五、问题解答及实验结果', exp2_pos)
print(f'实验二五部分位置: {five_exp2}')

# Find the end of 五 paragraph
five_end = content.find('</w:p>', five_exp2) + 6
print(f'五段落结束: {five_end}')

# Find 六 section for exp2
six_exp2 = content.find('六、实验体会和收获', five_end)
print(f'实验二六部分位置: {six_exp2}')

# Find the <w:p tag for 六 paragraph
six_p_start = content.rfind('<w:p', 0, six_exp2)
print(f'六段落开始: {six_p_start}')

# XML escape
def xml_escape(text):
    return text.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;')

# Generate simple paragraphs matching the original format
def make_para(text):
    """Generate paragraph in simple format matching exp1."""
    escaped = xml_escape(text)
    return f'<w:p w:rsidR="008E0228" w:rsidRDefault="008E0228"><w:r><w:rPr><w:szCs w:val="21"/></w:rPr><w:t>{escaped}</w:t></w:r></w:p>'

def make_para_chinese(text):
    """Chinese text paragraph."""
    escaped = xml_escape(text)
...
for line in content_lines:
    ...
    if is_sql:
        ...
        new_paras.append(make_para_chinese(line))

new_content = ''.join(new_paras)

# Insert: replace content between five_end and six_p_start
new_doc = content[:five_end] + new_content + content[six_p_start:]

# Verify tag counts
open_tags = new_doc.count('<w:p ')
close_tags = new_doc.count('</w:p>')
print(f'修改后: <w:p = {open_tags}, </w:p> = {close_tags}')
...
# Now add experience (第六部分)
# Find 六 paragraph end
with open(f'{dst}/word/document.xml', 'r', encoding='utf-8') as f:
    content2 = f.read()

six_pos = content2.find('六、实验体会和收获', exp2_pos)
six_end = content2.find('</w:p>', six_pos) + 6

# Find exp3 title position
exp3_pos = content2.find('实验项目名称', six_pos + 50)
exp3_p_start = content2.rfind('<w:p', 0, exp3_pos)

print(f'六段落结束: {six_end}')
...
exp_paras = [make_para_chinese(line) for line in exp_lines]
exp_content = ''.join(exp_paras)

new_doc2 = content2[:six_end] + exp_content + content2[exp3_p_start:]

# Verify again
open_tags2 = new_doc2.count('<w:p ')
close_tags2 = new_doc2.count('</w:p>')
print(f'添加六后: <w:p = {open_tags2}, </w:p> = {close_tags2}')
